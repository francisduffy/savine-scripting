...
class quickStack {
  ...
  public:
    ...
    inline iterator begin() { return reverse_iterator<T*>(myData + mySp); }

    inline const_iterator begin() const { return reverse_iterator<const T*>(myData + mySp); }

    inline iterator end() { return reverse_iterator<T*>(myData); }
    ...
    inline T& top() { return myData[mySp - 1]; }

    inline const T& top() const { return myData[mySp - 1]; }

    // Random access
    inline T& operator[](const size_t i) { return myData[mySp - 1 - i]; }
    ...
    void pop(const size_t n) { mySp -= n; }
    ...
    size_t size() const { return mySp; }

    size_t capacity() const { return mySize; }
