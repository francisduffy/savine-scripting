...
struct Model
{
    ...
    //  Apply the model SDE
    virtual void applySDE(const vector<double>& G, //  Gaussian numbers, dimension dim()
...
inline void simpleBsScriptVal(const Date& today,
                              const double spot,
                              const double vol,
                              const double rate,
                              ...
                              vector<double>& varVals)
{
    ...
    if (fuzzy)
        eval = prd.buildFuzzyEvaluator<double>(maxNestedIfs, defEps);
    else
        eval = prd.buildEvaluator<double>();

    //  Initialize model and random generator
    BasicRanGen random(seed);
    unique_ptr<Model<double>> model;
    if (normal)
        model.reset(new SimpleBachelier<double>(today, spot, vol, rate));
    else
        model.reset(new SimpleBlackScholes<double>(today, spot, vol, rate));
