...
#include "utilities/functDomain.h"
...
/*
   ...
   continuous intervals, not necessarily to compute all intervals accurately. For example, is x's domain is {0} and y's
   domain is (-inf, inf) then xy's domain is {0}, but if x and y have domain (-inf, inf), then xy's domain is (-inf,
   inf), even if it turns out that y=x.
        ...
        In addition, when fuzzy processing is requested, the processor sets the continuous/discrete flag on the equal
   and superior nodes and if discrete, the left and right interpolation bounds
  ...
  public:
    ...
    void visitEqual(NodeEqual& node) override {
        ...
        ++iii;
        {
            ...
            if (node.myDiscrete)
                ofs << "Node lB, rB = " << node.myLb << "," << node.myRb << endl;
        }
#endif
        // End of dump
    ...
    void visitSuperior(NodeSuperior& node) override { visitSupT<true>(node); }
    ...
    void visitIf(NodeIf& node) override {
        ...
        if (cp == alwaysTrue) {
            ...
            for (size_t i = 0; i < node.myAffectedVars.size(); ++i)
                domStore0[i] = myVarDomains[node.myAffectedVars[i]];
            ...
            for (size_t i = 0; i < node.myAffectedVars.size(); ++i)
                domStore1[i] = move(myVarDomains[node.myAffectedVars[i]]);

            // Reset variable domains
            for (size_t i = 0; i < node.myAffectedVars.size(); ++i)
                myVarDomains[node.myAffectedVars[i]] = move(domStore0[i]);
            ...
            // Merge domains
            for (size_t i = 0; i < node.myAffectedVars.size(); ++i)
                myVarDomains[node.myAffectedVars[i]].addDomain(domStore1[i]);
        }
    }
