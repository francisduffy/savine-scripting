...
class Evaluator : public constVisitor
{
  ...
  public:
    ...
    const vector<T>& varVals() const { return myVariables; }

    // Set generated scenarios and current event

    // Set reference to current scenario
    void setScenario(const Scenario<T>* scen) { myScenario = scen; }

    // Set index of current event
    void setCurEvt(const size_t curEvt) { myCurEvt = curEvt; }
...
#define ONEMINUSEPS 0.999999999999
    ...
    void visitTrue(const NodeTrue& node) override { myBstack.push(true); }
    void visitFalse(const NodeFalse& node) override { myBstack.push(false); }
    ...
    void visitConst(const NodeConst& node) override { myDstack.push(node.val); }

    // Scenario related
    void visitSpot(const NodeSpot& node) override { myDstack.push((*myScenario)[myCurEvt].spot); }
};
